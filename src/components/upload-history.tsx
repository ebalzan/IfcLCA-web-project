...
export function UploadHistory({ uploads, currentVersion, filters }: UploadHistoryProps) {
  ...
  const [status, setStatus] = React.useState<('Processing' | 'Completed' | 'Failed')[]>(
    filters.status
  )
          ...
          <DropdownMenuContent className="w-[180px]">
            <DropdownMenuCheckboxItem
              ...
              onCheckedChange={checked =>
                setStatus(
                  checked ? [...status, 'Processing'] : status.filter(s => s !== 'Processing')
                )
              }>
              Processing
            </DropdownMenuCheckboxItem>
            <DropdownMenuCheckboxItem
              ...
              onCheckedChange={checked =>
                setStatus(
                  checked ? [...status, 'Completed'] : status.filter(s => s !== 'Completed')
                )
              }>
              Completed
            </DropdownMenuCheckboxItem>
            <DropdownMenuCheckboxItem
              ...
              onCheckedChange={checked =>
                setStatus(checked ? [...status, 'Failed'] : status.filter(s => s !== 'Failed'))
              }>
              Failed
            </DropdownMenuCheckboxItem>
          </DropdownMenuContent>
        ...
        {filteredUploads.map(upload => (
          <Card key={upload.id}>
            <CardHeader>
              ...
              <CardDescription>Uploaded on {format(upload.uploadDate, 'PPP')}</CardDescription>
            </CardHeader>
            <CardContent>
            ...
            <CardFooter className="flex justify-between">
              <Button variant="outline" onClick={() => handleCompare(upload.id)}>
                Compare
              </Button>
